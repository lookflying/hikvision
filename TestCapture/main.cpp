...
//}
//
//void writeStreamPipe(int channel_id, unsigned char* buf, unsigned int len, int timestamp){
//	HANDLE		hPipe;
//	BOOL   fConnected = FALSE; 
//	wchar_t name[FILE_NAME_MAX_LEN];
//	wsprintf(name, TEXT("\\\\.\\Pipe\\channel%d.264"), channel_id);
...
void oriStreamHandler(UINT channel_id, void* context){
	...
	//	
	//	EncodedStream::getStream(channel_id)->write264(cur->p_payload, (unsigned int)cur->i_payload);
	//	EncodedStream::getStream(channel_id)->writePipe264(cur->p_payload, (unsigned int)cur->i_payload);
...
int main(int argc, char** argv){
	...
	//rtmp_publisher = new RtmpPublisher("rtmp://127.0.0.1/live/test2");
